...
async def get_packages(workspace_path: str, host_profile: str, build_profile: str, remote: Optional[str] = None) -> List[ConanPackage]:
    ...
    try:
        if os.path.exists(conanfile_txt):
            ...
            raise HTTPException(status_code=404, detail="No conanfile found in workspace")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error parsing conanfile: {str(e)}")
...
async def parse_conanfile_py(file_path: str, host_profile: str, build_profile: str, remote_name: Optional[str] = None) -> List[ConanPackage]:
    ...
    try:
        ...
        if conanfile_class:
            ...
            profile_host = conan_api.profiles.get_profile([host_profile], {}, {}, {}, None)
            profile_build = conan_api.profiles.get_profile([build_profile], {}, {}, {}, None)
            
            # Initialize the conanfile with profile (this is crucial for dynamic requirements)
            initialize_conanfile_profile(conanfile, profile_build=profile_build, profile_host=profile_host, base_context=CONTEXT_BUILD, is_build_require=False)
        ...
        raise HTTPException(status_code=500, detail=f"Error parsing conanfile.py: {str(e)}")
            ...
            for filename in os.listdir(profiles_path):
                if filename not in ['.', '..'] and not filename.startswith('.'):
                    ...
                    if os.path.isfile(profile_path):
                        profiles.append(ConanProfile(name=filename, path=profile_path))
        
        # Always include "default" profile as it's implicit in Conan
        if not any(p.name == "default" for p in profiles):
            profiles.insert(0, ConanProfile(name="default", path="default"))
...
async def install_packages(workspace_path: str, request: InstallRequest):
    ...
    try:
        ...
        # Get profiles
        profile_host = conan_api.profiles.get_profile([request.host_profile], {}, {}, {}, None)
        profile_build = conan_api.profiles.get_profile([request.build_profile], {}, {}, {}, None)
        ...
        raise HTTPException(status_code=500, detail=f"Error during installation: {str(e)}")
...
async def install_package(request: InstallPackageRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error during package installation: {str(e)}")
...
async def create_profile(request: ProfileCreateRequest):
    ...
    try:
        ...
        profile = conan_api.profiles.get_profile([])  # Start with empty profile
...
async def upload_packages_task(workspace_path: str, request: UploadRequest):
    ...
    try:
        ...
        try:
            ...
            raise Exception(f"Failed to load profiles: host={request.host_profile}, build={request.build_profile}")
        ...
        if hasattr(conanfile, 'requires'):
            for r in conanfile.requires.values():
                ...
                try:
                    ...
                    package_list = conan_api.list.select(ref_pattern, profile=profile_host)
                    
                    if package_list.recipes:
                        print(f"Uploading {r.ref}")
                        conan_api.upload.upload_full(package_list, remote, remotes, dry_run=False)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail=f"Failed to load host profile '{request.host_profile}': {str(e)}")
            
            package_list = conan_api.list.select(ref_pattern, profile=profile_host)
            
            if not package_list.recipes:
                raise HTTPException(status_code=404, detail=f"Package {request.package_ref} not found in local cache")
        ...
        raise HTTPException(status_code=500, detail=f"Error uploading package: {str(e)}")
...
async def check_package_availability(package_ref: str, host_profile: str, build_profile: str, remote_name: Optional[str] = None) -> PackageAvailability:
    ...
    try:
        ...
        conan_api.graph.analyze_binaries(
            deps_graph, 
            build_mode=['never'],  # Don't build anything, just analyze what exists
            remotes=remotes, 
            update=None,
            lockfile=None,
            build_modes_test=None,
        ...
        for node in deps_graph.nodes:
            if node.ref and str(node.ref).startswith(package_ref.split('/')[0]):  # Match package name
                target_node = node
                break
        ...
        recipe_status = str(target_node.recipe) if target_node.recipe else "unknown"
        binary_status = str(target_node.binary) if target_node.binary else "unknown"
            ...
            try:                
                ...
                for remote in remotes:
                    ...
                    try:
                        ...
                        print(f"Error checking remote availability for recipe {recipe_ref}: {e}")
                        pass  # Binary not found on this remote, try next

                    # Check if binary exists for this package ID
                    try:
                        package_ref: PkgReference = PkgReference(target_node.ref, target_node.package_id)
                        package_revisions = conan_api.list.package_revisions(package_ref, remote)
                        if package_revisions:
                            remote_binary_available = True
                    except Exception as e:
                        print(f"Error checking remote availability for package {package_ref}: {e}")
                        pass  # Binary not found on this remote, try next
                ...
                print(f"Error checking remote availability for {target_node.ref}: {e}")
